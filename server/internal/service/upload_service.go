...
// UploadImage handles image file upload to COS and returns the object key
func (s *UploadService) UploadImage(file *multipart.FileHeader, bizType UploadBizType) (string, error) {
	// validate biz type
	if bizType != BizTypeImage && bizType != BizTypeAvatar && bizType != BizTypeTrip {
	...
	_, err = s.cosClient.Object.Put(ctx, objectKey, bytes.NewReader(content), nil)
	if err != nil {
		return "", fmt.Errorf("上传到COS失败: %w", err)
	}
